...
public class Day21 {
    ...
    public Object part1() {
        ...
        List<Set<String>> afterOne = new ArrayList<>();
        for (Set<String> pathsForSingleEntry : pathsOnNumpad) {
            ...
            Set<String> shortest = findShortest(consolidated);
            afterOne.add(shortest);
        }

        List<Set<String>> afterTwo = new ArrayList<>();
        for (Set<String> pathsForSingleEntry : afterOne) {
            List<Set<String>> one = findPathsOnDir(pathsForSingleEntry.stream().toList(), dirPaths);
            List<String> consolidated = one.stream().flatMap(Collection::stream).distinct().toList();
            Set<String> shortest = findShortest(consolidated);
            afterTwo.add(shortest);
        }

        List<Integer> lengths = new ArrayList<>();
        for (Set<String> pathsForSingleEntry : afterTwo) {
